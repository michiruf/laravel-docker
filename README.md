...
The container clones `GIT_URL`, provisions the application, and from then on
checks upstream every minute — a push to the repository is live one cron tick
later. See [examples/autopull/compose.yml](examples/autopull/compose.yml) for
the full stack with mysql and redis.
